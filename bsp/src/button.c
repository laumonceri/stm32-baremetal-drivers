...
void Button_Init(const gpio_pin_cfg_t *button_cfg) {
  // Buttons are inputs for the board, so we configure the pin as input with
  // pull-up (active low button)
  (void)GPIO_ConfigPin(button_cfg);
}

bool Button_EventPending(void) { return button_event; }

void Button_ClearEvent(void) { button_event = false; }
