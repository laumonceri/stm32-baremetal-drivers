...
/* The handle must be UART_DeInit'd first, so ownership handoff is always
 * explicit. */
int UART_InstanceInUse(const uart_device_t *dev, const uart_handle_t *h) {
  int idx = UART_RegistryIndex(dev->uart.irq);
  return idx >= 0 && uart_handle_registry[idx] != NULL &&
         uart_handle_registry[idx] != h;
}
...
void USART1_IRQHandler(void) { UART_DispatchIRQ(USART1_IRQn); }

void USART2_IRQHandler(void) { UART_DispatchIRQ(USART2_IRQn); }
...
void UART_ReadString_RingBuffer(uart_handle_t *h, char *s_received,
                                int max_len) {
  int idx = 0;
  char c;
